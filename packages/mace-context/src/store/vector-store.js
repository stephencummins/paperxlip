const GEMINI_EMBED_URL = "https://generativelanguage.googleapis.com/v1beta/models/text-embedding-005:embedContent";
/**
...
export class VectorStore {
    dbUrl;
    apiKey;
    pool; // pg.Pool — imported dynamically to avoid ESM issues
    constructor(dbUrl) {
        this.dbUrl = dbUrl;
    ...
    async upsertDocument(doc) {
        ...
        // Upsert document
        const docResult = await pool.query(`INSERT INTO mace_documents (company_id, source_type, source_url, title, content, mime_type, metadata)
       ...
       RETURNING id`, [
        ...
        for (let i = 0; i < chunks.length; i++) {
            await pool.query(`INSERT INTO mace_chunks (document_id, company_id, content, chunk_index, embedding_json, metadata)
         VALUES ($1, $2, $3, $4, $5, $6)`, [
                documentId,
                doc.companyId,
                chunks[i],
        ...
        console.log(`[mace-context] Upserted ${chunks.length} chunks for "${doc.title}" (company: ${doc.companyId})`);
